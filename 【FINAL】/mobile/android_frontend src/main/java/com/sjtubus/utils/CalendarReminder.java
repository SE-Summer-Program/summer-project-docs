...
public class CalendarReminder {
    ...
    private static String CALENDARS_NAME = "SJTU-Calendar";
    private static String CALENDARS_ACCOUNT_NAME = "SJTU-User";
    private static String CALENDARS_ACCOUNT_TYPE = "Normal-User";
    ...
    public static void addCalendarEventRemind(Context context, @NonNull String title, String description, long begintime, long endtime, int remind_minutes, onCalendarRemindListener callback){
        ...
        if(TextUtils.isEmpty(event_id)){
            ...
            if (newEvent == null) {
                // 添加日历事件失败直接返回
                if(null != callback){
                    callback.onFailed(onCalendarRemindListener.Status.EVENT_ERR);
                }
                return;
            }
